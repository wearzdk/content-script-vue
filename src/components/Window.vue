...
onMounted(() => {
  ...
  // props 覆盖
  if (props.top) initY = props.top;

  if (props.left) initX = props.left;
  ...
  const { style } = useDraggable(dragHead, {
    ...
    preventDefault: true,
    onStart: () => {
      if (isOnPinned.value) return false;
    },
    onMove: (e) => {
      // 边界检测
      const { x, y } = e;
...
<template>
  <div ref="contentBox" class="kf-crx-content-box" :style="dragStyle">
    <section ref="dragHead" class="lz-panel-title" :style="{
